"""
One 2 One Monthly Linear Regression
"""
...
modelPathPair = {
    # "linear regression": (LinearRegression, config.RES_FOLDER_PATH.O2O_M_LR.MAIN, dict()),
    # "random forest regressor": (RandomForestRegressor, config.RES_FOLDER_PATH.O2O_M_RFR.MAIN, dict()),
    "svm": (LinearSVR, config.RES_FOLDER_PATH.O2O_M_SVM.MAIN, {"random_state": config.RANDOM_SEED, "max_iter": 2000}),
}
    ...
    for colInd, seqTotal, trainRatioAll, xTrainTotal, yTrainTotal, xTestTotal, yTestTotal, data \
        ...
        for d in range(1, 7):
            ...
            for i in range(len(yPred)):
                mon = trainRatioAll + 2 + i - d
                yy = models[mon % 12].predict([seqTotal[mon]])
                for t in range(1, d):
                    yy = models[(mon + t) % 12].predict(yy.reshape(-1, 1))
                yPred[i] = yy[0]
            # conclude result
            if True:
