...
modelPathPair = {
    "linear regression": (LinearRegression, config.RES_FOLDER_PATH.M2O_S_LR.MAIN, dict()),
    "random forest regressor": (RandomForestRegressor, config.RES_FOLDER_PATH.M2O_S_RFR.MAIN, dict()),
    "svm": (LinearSVR, config.RES_FOLDER_PATH.M2O_S_SVM.MAIN,
            {"random_state": config.RANDOM_SEED, "max_iter": 100000, "tol": 1e-3}),
    "lasso": (Lasso, config.RES_FOLDER_PATH.M2O_S_LASSO.MAIN, dict()),
}
